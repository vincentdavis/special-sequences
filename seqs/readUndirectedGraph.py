...
def vertex(G, v):
    ...
    if v in G:
        raise GraphFormatError("Duplicate vertex %s").with_traceback(str(v))
    G[v] = {}
...
def edge(G, u, v, e):
    ...
    if u not in G:
        raise GraphFormatError(
            "Unexpected vertex {0!s} in edge to {1!s}".format(str(u), str(v))
        )
    if v not in G:
        raise GraphFormatError(
            "Unexpected vertex {0!s} in edge from {1!s}".format(str(v), str(u))
        )
    G[u][v] = G[v][u] = e
...
def readEdgeList(lines):
    ...
    for line in filter(None, lines):
        ...
        if len(words) < 2 or len(words) > 3:
            raise GraphFormatError(
                'Wrong number of words in edge list: "{0!s}"'.format(line)
            )
        if len(words) == 3 and words[1] != "-":
            raise GraphFormatError(
                'Unrecognized edge type "{0!s}" in edge list'.format(words[1])
            )
        u, v = words[0], words[-1]
        if u not in G:
            vertex(G, u)
...
def readNodeEdgeList(lines):
    ...
    def namedEdge(line):
        ...
        if id in EdgeNames:
            raise GraphFormatError(
                'Edge name "{0!s}" used twice in node edge list'.format(id)
            )
    ...
    for line in lines:
        if line.startswith("// "):
            try:
                ...
                raise GraphFormatError(
                    'Unrecognized section "{0!s}" in node edge list'.format(line[3:])
                )
        else:
            action(line)
...
def readGraphML(lines):
    ...
    def start_element(name, attrs):
        ...
        if len(context) == 1:
            if name != "graphml":
                raise GraphFormatError(
                    'Unrecognized outer tag "{0!s}" in GraphML'.format(name)
                )
...
def readGraph6(str):
    ...
    if len(data) != nd:
        raise GraphFormatError(
            "Expected {0:d} bits but got {1:d} in graph6".format(
                n * (n - 1) // 2, len(data) * 6
            )
        )
...
def readLeda(lines):
    ...
    for i in range(m):
        ...
        if len(words) < 4:
            raise GraphFormatError(
                "Too few fields in LEDA.GRAPH edge {0:d}".format((i + 1))
            )
        ...
        if not reversal:
            raise GraphFormatError(
                "Edge {0:d} is directed in LEDA.GRAPH".format((i + 1))
            )
        if source < target:
